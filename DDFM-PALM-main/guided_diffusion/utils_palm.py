...
from matlab_imresize_master.imresize import imresize

print("utilisation de utils_palm")
...
def FusionPALM(y1, y2, c, tau1, tau2, tau3, tau4, d, m_iteration, f_np):
    ...
    #yint = resize(y1, scale_factors=(d,d), interp_method=cubic)
    yint = imresize(y1, scalar_scale=d, method='bicubic')
    #f_np_resize = resize(f_np, out_shape=yint.shape, interp_method=cubic)
    f_np_resize = imresize(f_np, output_shape=yint.shape, method='bicubic')
