...
# Dictionnaire global pour enregistrer les types de samplers disponibles
__SAMPLER__ = {}

# Décorateur pour enregistrer un sampler sous un nom donné
...
# Récupération d'un sampler à partir du nom
def get_sampler(name: str):
    if __SAMPLER__.get(name, None) is None:
        raise NameError(f"Name {name} is not defined!")
    return __SAMPLER__[name]

# Création d'une instance de sampler avec les bons paramètres
...
# Classe principale pour le processus de diffusion gaussien
class GaussianDiffusion:
    def __init__(self,
        ...
        # Pré-calcul de coefficients utiles au sampling
        ...
        # Calculs pour le posterior q(x_{t-1} | x_t, x_0))
        ...
        # Processeurs de moyenne/variance du sampling (model wrappers)
    ...
    # Moyenne et variance de q(x_t | x_0)
    ...
    # Ajout de bruit : x_t = sqrt(alpha_t) * x0 + sqrt(1 - alpha_t) * noise
    ...
    # Moyenne/variance du posterior q(x_{t-1} | x_t, x_0)
    ...
    def p_sample_loop(self,
        ...
        for idx in pbar:
            ...
            out = self.p_sample(x=img, t=time, model=model, infrared=I, visible=V, lamb=lamb, rho=rho, palm_init=palm_init)
            ...
            if record and idx % 1 == 0:
                ...
                imsave(os.path.join(file_path, f"x_{str(idx).zfill(4)}.png"), temp_img)
            if not record:  # Affichage du résultat final si pas d'enregistrement
                temp_img = img.detach().cpu().squeeze().numpy()
                temp_img = np.transpose(temp_img, (1, 2, 0))
                temp_img = cv2.cvtColor(temp_img, cv2.COLOR_RGB2YCrCb)[:, :, 0]
                temp_img = (temp_img - np.min(temp_img)) / (np.max(temp_img) - np.min(temp_img))
...
class DDIM(SpacedDiffusion):
    ...
    def p_sample(self, model, x, t, infrared, visible, lamb, rho, palm_init=None, eta=0.0):
        out = self.p_mean_variance(model, x, t)
        # Extraction du canal Y
        x_0_hat_ycbcr = rgb_to_ycbcr(out['pred_xstart']) / 255
        x_0_hat_y = torch.unsqueeze((x_0_hat_ycbcr[:, 0, :, :]), 1)  # (B, 1, H, W)
        assert x_0_hat_y.shape[1] == 1
        # Fusion avec PALM
        x_0_hat_y_BF = fusion_onestep(x_0_hat_y, palm_init=palm_init)
        # Mise en forme correcte : (1, 1, H, W)
        if isinstance(x_0_hat_y_BF, np.ndarray):
            x_0_hat_y_BF = torch.from_numpy(x_0_hat_y_BF).float().to(x.device)
        if x_0_hat_y_BF.ndim == 2:
            x_0_hat_y_BF = x_0_hat_y_BF.unsqueeze(0).unsqueeze(0)  # (1, 1, H, W)
        elif x_0_hat_y_BF.ndim == 3:
            if x_0_hat_y_BF.shape[0] != 1:
                x_0_hat_y_BF = x_0_hat_y_BF.unsqueeze(0)  # (1, H, W) → (1, 1, H, W)
            else:
                x_0_hat_y_BF = x_0_hat_y_BF.unsqueeze(1)  # (1, H, W) → (1, 1, H, W)
        # Resize vers la taille originale
        H, W = visible.shape[-2:]
        x_0_hat_y_BF_resized = F.interpolate(
            x_0_hat_y_BF, size=(H, W), mode='bicubic', align_corners=False
        )
        # Remplacement du canal Y
        x_0_hat_ycbcr[:, 0, :, :] = x_0_hat_y_BF_resized.squeeze(1)
        # Reconstruction RGB
        out['pred_xstart'] = ycbcr_to_rgb(x_0_hat_ycbcr * 255)
        # Recalcul de epsilon (bruit prédit)
        ...
        # Calcul du sigma pour sampling DDIM
        sigma = (
            eta
